...
public class SampleMecanumDrive extends MecanumDrive {
    ...
    public static PIDCoefficients HEADING_PID = new PIDCoefficients(-5.6, 0, 0.01);

    public static double LATERAL_MULTIPLIER = 1.153363671773139;
    ...
    public void lift(int target, double max){
        ...
        while(Math.abs(target - lift.getCurrentPosition()) > Math.abs(encoderLeft)){
            speed = max;
            lift.setPower(speed);
            lift2.setPower(speed);
        }

        while((Math.abs(lift.getCurrentPosition()) < Math.abs(target)) && Math.abs(speed) > 0){
